...
# b&b algorithm with prune plicy learned with last iteration
#-----------------------------------------------------------
...
def binaryPro_policy(K0,L0,R_min_C0,P_max_D0,P_max_C0,h_CD0,h_D0,h_CB0,h_DB0,index,model,scaler): 
    ...
    while(len(nodeStack) > 0):
        ...
        if node.exitflag < 0 : # no feasible solution
            ...
            if node.fathomed_flag != 1:
                ...
                tmpFile.writelines(['\n'])
                tmpFile.close()
                # use prune plicy learned with last iteration
